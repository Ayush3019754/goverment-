...
const BATCH_SIZE = 12; // Ek baar me kitne cards dikhane hain
let currentIndex = 0;
...
function filterLocalServices() {
  ...
  // Set global array and reset pagination index
  servicesToDisplay = matches;
  currentIndex = BATCH_SIZE;

  // Initial render (isAppending = false)
  renderCards(servicesToDisplay, false);
}

// ==========================================
// 3. CARDS RENDERER & LOAD MORE LOGIC
// ==========================================
function renderCards(items, isAppending = false) {
  const grid = document.getElementById('stateServicesGrid');

  if (!isAppending) {
    grid.innerHTML = "";
  } else {
    // Purana Load More Button remove karein taaki duplicate buttons grid me na rahein
  ...
  // Current batch slice karein
  const start = isAppending ? currentIndex - BATCH_SIZE : 0;
  const itemsToRender = items.slice(start, currentIndex);
